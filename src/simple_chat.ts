...
function getElementAndCheck<T extends HTMLElement>(id: string): T {
...
class ChatUI {
  ...
  private handleContextChange = async () => {
    const fileInput = getElementAndCheck<HTMLInputElement>("context");
    fileInput.addEventListener("change", async () => {
      try {
        const file = fileInput.files ? fileInput.files[0] : null;
        if (file) {
          this.context = await file.text();
          // Further processing of the text content can be done here
        }
      } catch (error) {
        throw new Error("Error handling file change: " + error);
        console.error(error);
      }
    });
  }
  ...
  private async asyncGenerate() {
    ...
    const promptWithContext = this.context + " " + prompt;
    try {
      if (this.selectedModel == "Local Server") {
        const output = await this.localChat.generate(promptWithContext, callbackUpdateResponse);
        ...
        const output = await this.chat.generate(promptWithContext, callbackUpdateResponse);
        this.updateLastMessage("left", output);
